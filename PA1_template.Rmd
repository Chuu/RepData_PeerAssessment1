...
* Now we redo the analysis from the first section of this study.
```{r, echo=TRUE, results="asis"}
generateStepHist(pa1.DataFilled)
generateStepMeanMedianByDateTable(pa1.DataFilled)
```
The mean is the same, the median is now equal to the mean, when previously it was 1 step off.  What we would expect to happen completely depends on the distribution of the NA values, which we did not stop to consider.  The fact the median is now equal to the mean is interesting but not significant since it only represents a very small step.
