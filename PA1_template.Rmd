...
```{r, echo=TRUE, results="asis"}
generateStepMeanByDateTable <- function(table)
  {
    pa1.meanByDate <- aggregate(steps ~ date, data=table, 
...
generateStepMeanByDateTable(pa1.data)
```
...
generateStepMeanByDateTable(pa1.DataFilled)
```
