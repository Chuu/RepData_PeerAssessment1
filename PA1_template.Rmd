...
* First, we're going to aggregate the data by interval, then change the interval column to a time data type
```{r, echo=TRUE, results="asis"}
...
pa1.stepsByInterval["interval"] <- lapply(pa1.stepsByInterval["interval"], pa1.strToTime);
...
strftime(pa1.Interval, format="%H:%M:%S", usetz=TRUE);
```
...
* First, we are going to modify the `pa1.DataFilled` variable to include a column signifying if the date is a weekday or weekend, using the `weekday(...)` function.

```{r, echo=TRUE}
pa1.DataFilled["IsWeekday"] <- !(pa1.DataFilled$date %in% c("Saturday","Sunday"));
```